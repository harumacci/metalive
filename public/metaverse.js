...
function connectToServer() {
    ...
    socket.on('loginSuccess', (data) => {
        ...
        document.getElementById('playerName').textContent = data.nameID;
    });
...
function updatePlayerMovement(delta) {
    ...
    if (currentTime - lastUpdateTime > UPDATE_INTERVAL && localPlayer) {
        ...
        document.getElementById('position').textContent = `${localPlayerMesh.position.x.toFixed(1)}, ${localPlayerMesh.position.y.toFixed(1)}, ${localPlayerMesh.position.z.toFixed(1)}`;
    }
}
...
function initVoiceChat() {
    navigator.mediaDevices.getUserMedia({ audio: true })
        .then(stream => {
            ...
            peer.on('error', err => console.error('PeerJSエラー: ', err));
