...
// PeerJSのエンドポイントを明示的に保護
app.use('/peerjs', (req, res, next) => {
    // PeerJSのリクエストはPeerJSサーバーに委譲
    console.log('PeerJS request intercepted:', req.method, req.path);
    next();
});

// 静的ファイルの提供（PeerJSパスの後に設定）
...
// PeerJSサーバーの状態をログ出力
console.log('PeerJS server initialized with path: /peerjs');
